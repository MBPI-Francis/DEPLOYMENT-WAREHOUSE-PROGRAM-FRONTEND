...
def entry_fields(note_form_tab):
    ...
    preparation_form_frame = ttk.Frame(note_form_tab)
    preparation_form_frame.pack(fill=X, pady=10, padx=20)

    # Configure grid columns to make them behave correctly
    preparation_form_frame.grid_columnconfigure(0, weight=1)  # Left (Warehouse) stays at the left
    preparation_form_frame.grid_columnconfigure(1, weight=1)  # Right (Ref Number) is pushed to the right

    # Warehouse FRAME (Left-aligned)
    warehouse_frame = ttk.Frame(preparation_form_frame)
    ...
    checkbox_status_var = ttk.IntVar(value=1)  # Integer variable to store checkbox state (0 or 1)
    ...
    refno_frame = ttk.Frame(preparation_form_frame)
    ...
    # RM CODE FRAME
    rmcode_frame = ttk.Frame(preparation_form_frame)
    ...
    qty_prepared_formatter = NumericInputFormatter(qty_prepared_entry, qty_prepared_var)
    ...
    #  This calls the auto format class to format the value automatically
    qty_return_formatter = NumericInputFormatter(qty_return_entry, qty_return_var)
    ...
    date_frame = ttk.Frame(preparation_form_frame)
    ...
    btn_add_preparation = ttk.Button(
        preparation_form_frame,
        text="+ Add",
        command=submit_data,
    )
    btn_add_preparation.grid(row=2, column=0, columnspan=3, pady=0, padx=400, sticky=NSEW)
    ToolTip(btn_add_preparation, text="Click this add button to add the entry to the list")
    ...
    bind_shift_enter_to_all_children(preparation_form_frame, lambda e: btn_add_preparation.invoke())
