...
def create_soh_whse_excel(date_entry_value, data):
    ...
    # Function to create a WHSE sheet dynamically
    def create_whse_sheet(whse_num):
        sheet_name = f"WHSE{whse_num}"
        sheet = wb.create_sheet(sheet_name)

        # Set dynamic header based on warehouse number
        ...
        # Insert a row for the report date and set its style
        sheet["A1"] = f"{wh_date}"
        ...
        for record in data:
            try:
                # Safely parse the warehouse number to handle integers
                record_whse_num = int(record.get("warehousenumber"))
            except (ValueError, TypeError):
                continue  # Skip if warehouse number is missing or invalid

            # Ensure 'warehousenumber' matches current dynamically built sheet
            if record_whse_num == whse_num:
                row = [
    ...
    # -------------------------------------------------------------
    # NEW DYNAMIC SHEET CREATION LOGIC
    # -------------------------------------------------------------
    unique_whse_nums = set()

    # 1. Loop through all data to find which warehouse numbers exist
    for record in data:
        wh_num = record.get("warehousenumber")
        if wh_num is not None:
            try:
                unique_whse_nums.add(int(wh_num))
            except (ValueError, TypeError):
                pass

    # 2. Sort them numerically so they appear in order (e.g. 1, 2, 3...) and create sheets
    for whse_num in sorted(unique_whse_nums):
        create_whse_sheet(whse_num)
    # -------------------------------------------------------------
    ...
    if file_path:
        try:
            ...
            if excel_password and WIN32_AVAILABLE:
                excel_app = None
                try:
                    # Use DispatchEx to create a completely SEPARATE background instance of Excel
                    # This prevents interfering with the user's already opened Excel workbooks!
                    excel_app = win32.DispatchEx("Excel.Application")

                    excel_app.Visible = False  # Keep this specific instance hidden
                    excel_app.DisplayAlerts = False  # Suppress alerts

                    # Force STRICT Windows backslashes using normpath
                    abs_file_path = os.path.normpath(os.path.abspath(file_path))

                    # Open the workbook using the strict absolute path
                    ...
                    if excel_app:
                        excel_app.Quit()  # This will now ONLY quit the hidden instance, not your personal files!
