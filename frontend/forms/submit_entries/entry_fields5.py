...
# Matches XML 1.0 illegal control characters. openpyxl does not sanitize these
# automatically; if they slip through from upstream API data (e.g. a stray
# control byte in rmcode/status), the worksheet XML becomes invalid and Excel
# reports "we found a problem with some content" / catastrophic failure.
...
def _sanitize_xml_value(value):
    """Strip characters that are illegal in XML 1.0 from a cell value.

    Leaves numbers and other non-string types untouched.
    """
...
def create_soh_whse_excel(date_entry_value, data):
    ...
    try:
        ...
        for record in api_data:
            notes_sheet.append([
                _sanitize_xml_value(record.get("product_code", "")), # Use .get() for safer access
                _sanitize_xml_value(record.get("lot_number", "")),
                _sanitize_xml_value(record.get("product_kind_id", ""))
            ])
    ...
    def create_whse_sheet(sheet_name, wh_num):
        sheet = wb.create_sheet(sheet_name)

        # Build header dynamically based on the warehouse number
        wh_header = f"WHSE #{wh_num} - Excess"
        ...
        for record in data:
            # Ensure 'warehousenumber' exists and matches this sheet's warehouse number
            if record.get("warehousenumber") == wh_num:
                status_value = record.get("status", "") or ""
                row = [
                    _sanitize_xml_value(record.get("rmcode", "")),  # rmcode
                    "",  # No of bags (blank)
                    "",  # qty per packing (blank)
                    "",  # Excess column (blank)
                    float(record.get("new_beginning_balance", 0.0)),  # Total, ensure float
                    "" if status_value.lower() == "good" else _sanitize_xml_value(status_value),  # Store blank if status is 'good'
                    ""  # Drop list (blank)
                ]
                sheet.append(row)
        ...
        # Only add it if there is at least one data row to attach it to —
        # an empty sqref/range produces an invalid <dataValidation> element
        # and corrupts the worksheet XML ("catastrophic failure" on open).
        if sheet.max_row >= 3:
            dv = DataValidation(
                type="list",
    ...
    # Dynamically discover all unique warehouse numbers present in the data
    # and create a sheet for each one, instead of hardcoding WHSE1/2/4.
    warehouse_numbers = sorted({
        record.get("warehousenumber")
        for record in data
        if record.get("warehousenumber") is not None
    })

    for wh_num in warehouse_numbers:
        create_whse_sheet(f"WHSE{wh_num}", wh_num)
    ...
    if file_path:
        try:
            ...
            # Give the OS a moment to fully flush/release the file handle
            # before handing it to Excel's COM automation. Without this,
            # Workbooks.Open can race the openpyxl save and fail with
            ...
            if excel_password and WIN32_AVAILABLE:
                excel_app = None
                try:
                    # Use early-bound dispatch (reads Excel's type library) rather
                    # than plain win32.Dispatch. Late-bound dynamic dispatch does
                    # not reliably resolve keyword/named arguments on COM methods,
                    # which can itself cause "Open method of Workbooks class
                    # failed" if any are ever passed. EnsureDispatch avoids that
                    # class of problem entirely.
                    try:
                        excel_app = win32.gencache.EnsureDispatch("Excel.Application")
                    except AttributeError:
                        # Fallback if gencache isn't available in this environment
                        excel_app = win32.Dispatch("Excel.Application")

                    excel_app.Visible = False # Keep Excel hidden
                    excel_app.DisplayAlerts = False # Suppress alerts

                    # msoAutomationSecurityForceDisable: disables macros AND
                    # suppresses Trust Center / Protected View security prompts
                    # that DisplayAlerts does NOT cover. Those silent prompts are
                    # a common cause of "Open method of Workbooks class failed"
                    # when opening a file that was just written programmatically.
                    excel_app.AutomationSecurity = 3

                    # COM/Windows requires a Windows-style path with backslashes.
                    # asksaveasfilename can return forward slashes, which makes
                    # Workbooks.Open fail with "Open method of Workbooks class failed".
                    # Normalize to an absolute, backslash-style path before opening.
                    windows_file_path = os.path.normpath(os.path.abspath(file_path)).replace("/", "\\")

                    if not os.path.isfile(windows_file_path):
                        raise FileNotFoundError(
                            f"Saved workbook not found at expected path: {windows_file_path}"
                        )

                    # Open the workbook. Pass ONLY the path, positionally.
                    # Extra keyword args (UpdateLinks, ReadOnly, Notify, etc.)
                    # are deliberately omitted: on dynamically dispatched COM
                    # objects they can misbind to the wrong parameter slots and
                    # trigger this same generic Open failure. Retry a couple of
                    # times in case the file is still briefly locked (e.g. by
                    # antivirus scanning a freshly written file).
                    workbook = None
                    last_error = None
                    for attempt in range(3):
                        try:
                            workbook = excel_app.Workbooks.Open(windows_file_path)
                            break
