...
class TransferFormTable:
    def __init__(self, root):
        ...
        self.get_rm_code_api = self.shared_functions.get_rm_code_api()    # Instantiate the shared_function class
        self.get_status_api = self.shared_functions.get_status_api()    # Instantiate the shared_function class
    ...
    def edit_record(self, item):
        ...
        for idx, field in enumerate(fields):
            ...
            if field == "Raw Material":
                ...
                wh_from_entry = ttk.Combobox(self.edit_window, values=warehouse_names, state="disabled", width=20,
                                             font=self.shared_functions.custom_font_size)
                wh_from_entry.set(record[idx])  # Set current value in the combobox
                wh_from_entry.grid(row=idx, column=1, padx=10, pady=5, sticky=W)
                ToolTip(wh_from_entry, text="Select a warehouse")  # Tooltip
                ...
                wh_to_entry = ttk.Combobox(self.edit_window, values=warehouse_names, state="disabled", width=20,
                                           font=self.shared_functions.custom_font_size)
                wh_to_entry.set(record[idx])  # Set current value in the combobox+
                wh_to_entry.grid(row=idx, column=1, padx=10, pady=5, sticky=W)
                ToolTip(wh_to_entry, text="Select a warehouse")  # Tooltip
                ...
                validate_numeric_command = self.edit_window.register(TranferValidation.validate_numeric_input)
        ...
        def get_selected_warehouse_from_id():
            selected_name = wh_from_entry.get()
        ...
        save_button = ttk.Button(self.edit_window,
                                 text="Save",
