# -- VERSION 5.4 (Format date_encoded + Format date_reported + Conditional Summary + Exclusions Sheet + Wrap Text)
...
class NoteTable:
    ...
    def _perform_export_worker(self, params, save_path, loader):
        try:
            ...
            exclusions = {}
            df_exclusions = pd.DataFrame()  # Initialize an empty DataFrame for robustness
            try:
                ...
                print(f"WARNING: Could not read or process the exclusion file: {e}")
            ...
            # A. Format "Detailed Data" Sheet
            if "Detailed Data" in wb.sheetnames:
                ws_detailed = wb["Detailed Data"]
                self._apply_excel_formatting(ws_detailed, "Detailed Transaction Report")

                # --- Format the 'date_reported' column ---
                date_reported_col_idx = None
                for cell in ws_detailed[2]:
                    if cell.value == "date_reported":
                        date_reported_col_idx = cell.column
                        break
                if date_reported_col_idx:
                    col_letter = get_column_letter(date_reported_col_idx)
                    for cell in ws_detailed[col_letter]:
                        if cell.row < 3: continue
                        if isinstance(cell.value, datetime):
                            cell.number_format = 'MM/DD/YYYY'

                # --- NEW: Format the 'date_encoded' column ---
                date_encoded_col_idx = None
                for cell in ws_detailed[2]:
                    if cell.value == "date_encoded":
                        date_encoded_col_idx = cell.column
                        break
                if date_encoded_col_idx:
                    col_letter = get_column_letter(date_encoded_col_idx)
                    for cell in ws_detailed[col_letter]:
                        if cell.row < 3: continue
                        if isinstance(cell.value, datetime):
                            cell.number_format = 'MM/DD/YYYY'
            ...
            # B. Apply Conditional Formatting to "Daily RM Summary" Sheet
            if "Daily RM Summary" in wb.sheetnames:
                ws_summary = wb["Daily RM Summary"]
            ...
            # C. Calculate and Add Skipped Numbers Sheet
            if not df.empty:
            ...
            # D. Add and Format Excluded Numbers Sheet
            if not df_exclusions.empty:
                ws_excluded = wb.create_sheet("excluded_in_skipped_numbers_tab")
                for r in dataframe_to_rows(df_exclusions, index=False, header=True):
                    ws_excluded.append(r)
                self._apply_excel_formatting(ws_excluded, "Excluded Document Numbers")
                ws_excluded.column_dimensions['C'].width = 50
                for cell in ws_excluded['C']:
            ...
            # Save changes to the final file
            wb.save(save_path)
