...
#         """
#         Fetch filtered data from API, generate Excel file locally using pandas,
#         including detailed, overall daily summary, and per-warehouse daily summaries.
#         """
#         params = self._get_filter_params()
#         if params is None:
#             return
...
#             # Ensure 'date_computed' is a datetime object for proper grouping and formatting
#             df_detailed['date_computed'] = pd.to_datetime(df_detailed['date_computed'], errors='coerce')
#             # Drop rows where 'date_computed' became NaT
#             df_detailed.dropna(subset=['date_computed'], inplace=True)
#
#             if df_detailed.empty:
#                 messagebox.showinfo("No Valid Dates", "No valid date entries found for aggregation in the export data.", parent=self.root)
#                 return
#
#             # Create the overall daily RM summary (Sheet 2)
#             df_long_summary_overall = df_detailed.groupby(['rm_code', df_detailed['date_computed'].dt.date])['qty'].sum().reset_index()
#             df_long_summary_overall = df_long_summary_overall.rename(columns={'date_computed': 'Date'})
#             df_long_summary_overall['Date'] = df_long_summary_overall['Date'].apply(lambda x: x.strftime('%m/%d/%Y'))
#
#             df_pivot_summary_overall = df_long_summary_overall.pivot_table(
#                 index='rm_code',
#                 columns='Date',
#                 values='qty',
#                 fill_value=0
#             )
#             df_pivot_summary_overall = df_pivot_summary_overall.sort_index(ascending=True)
#             sorted_date_cols_overall = pd.to_datetime(df_pivot_summary_overall.columns, format='%m/%d/%Y', errors='coerce').sort_values().dropna()
#             sorted_date_col_names_overall = sorted_date_cols_overall.strftime('%m/%d/%Y').tolist()
#             df_pivot_summary_overall = df_pivot_summary_overall[sorted_date_col_names_overall]
#             df_pivot_summary_overall = df_pivot_summary_overall.reset_index()
#
#
#             # Create an in-memory binary stream for the Excel file
#             excel_buffer = BytesIO()
...
#                 df_detailed.to_excel(writer, index=False, sheet_name="Detailed Data")
#
#                 # Sheet 2: Overall Daily RM Summary
#                 df_pivot_summary_overall.to_excel(writer, index=False, sheet_name="Daily RM Summary")
#
#                 # Adjust column widths for overall summary sheet
#                 workbook = writer.book
#                 if 'Daily RM Summary' in writer.sheets:
#                     summary_sheet_overall = writer.sheets['Daily RM Summary']
#                     summary_sheet_overall.set_column(0, 0, 20)
#                     for i, col_name in enumerate(df_pivot_summary_overall.columns[1:]):
#                         summary_sheet_overall.set_column(i + 1, i + 1, 12)
#
#                 # --- NEW: Generate sheets per warehouse ---
#                 # Get unique warehouses from the detailed DataFrame
#                 # Using wh_name for sheet names and wh_number for potential sorting/identification
#                 unique_warehouses = df_detailed[['wh_name', 'wh_number']].drop_duplicates().sort_values(by='wh_name')
#
#                 for index, wh_row in unique_warehouses.iterrows():
#                     wh_name = wh_row['wh_name']
#                     wh_number = wh_row['wh_number'] # Get wh_number if needed for sheet name or internal logic
#
#                     # Sanitize warehouse name for sheet name (max 31 chars, no invalid chars)
#                     sheet_name = f"WHSE#{wh_number}"
...
#
#                     # Filter detailed data for the current warehouse
#                     df_wh_detailed = df_detailed[df_detailed['wh_name'] == wh_name].copy()
#
#                     if not df_wh_detailed.empty:
#                         # Generate pivot summary for the current warehouse
#                         df_long_summary_wh = df_wh_detailed.groupby(['rm_code', df_wh_detailed['date_computed'].dt.date])['qty'].sum().reset_index()
#                         df_long_summary_wh = df_long_summary_wh.rename(columns={'date_computed': 'Date'})
#                         df_long_summary_wh['Date'] = df_long_summary_wh['Date'].apply(lambda x: x.strftime('%m/%d/%Y'))
#
#                         df_pivot_summary_wh = df_long_summary_wh.pivot_table(
#                             index='rm_code',
#                             columns='Date',
#                             values='qty',
#                             fill_value=0
#                         )
#                         df_pivot_summary_wh = df_pivot_summary_wh.sort_index(ascending=True)
#                         sorted_date_cols_wh = pd.to_datetime(df_pivot_summary_wh.columns, format='%m/%d/%Y', errors='coerce').sort_values().dropna()
#                         sorted_date_col_names_wh = sorted_date_cols_wh.strftime('%m/%d/%Y').tolist()
#                         df_pivot_summary_wh = df_pivot_summary_wh[sorted_date_col_names_wh]
#                         df_pivot_summary_wh = df_pivot_summary_wh.reset_index()
#
#                         # Write to a new sheet
#                         df_pivot_summary_wh.to_excel(writer, index=False, sheet_name=sheet_name)
#
#                         # Adjust column widths for this warehouse sheet
#                         if sheet_name in writer.sheets:
#                             wh_sheet = writer.sheets[sheet_name]
#                             wh_sheet.set_column(0, 0, 20)
#                             for i, col_name in enumerate(df_pivot_summary_wh.columns[1:]):
#                                 wh_sheet.set_column(i + 1, i + 1, 12)
#                     # else:
#                     #     # Optional: Add a sheet indicating no data for this warehouse
#                     #     pd.DataFrame({"Message": [f"No valid data for {wh_name}."]}) \
...
class HistoricalSOHTable:
    def __init__(self, root):
        ...
        # --- Filter Fields ---
        ttk.Label(filter_button_frame, text="Date FROM", style="CustomLabel.TLabel").grid(row=0, column=0, padx=5,
                                                                                          pady=0, sticky=W)
        ttk.Label(filter_button_frame, text="Date TO", style="CustomLabel.TLabel").grid(row=0, column=1, padx=5, pady=0,
                                                                                        sticky=W)
        ttk.Label(filter_button_frame, text="Raw Material", style="CustomLabel.TLabel").grid(row=0, column=2,
                                                                                             padx=(8, 0), pady=0,
                                                                                             sticky=W)
        ttk.Label(filter_button_frame, text="Warehouse", style="CustomLabel.TLabel").grid(row=0, column=3, padx=(8, 0),
                                                                                          pady=0, sticky=W)
        ttk.Label(filter_button_frame, text="Status", style="CustomLabel.TLabel").grid(row=0, column=4, padx=(8, 0),
                                                                                       pady=0, sticky=W)
        ...
        rm_codes_data = self.shared_functions.get_rm_code_api(force_refresh=True)
        rm_names = ["All"] + [item["rm_code"] for item in rm_codes_data]
        self.rm_codes_combobox = Combobox(filter_button_frame, values=rm_names, state="normal", width=20,
        ...
        warehouses_data = self.shared_functions.get_warehouse_api()
        warehouse_names = ["All"] + [item["wh_name"] for item in warehouses_data]
        self.warehouse_combobox = Combobox(filter_button_frame, values=warehouse_names, state="readonly", width=13,
                                           font=self.shared_functions.custom_font_size)
        self.warehouse_combobox.set("All")
        self.warehouse_combobox.grid(row=1, column=3, padx=(10, 0), pady=0, sticky=W)
        ToolTip(self.warehouse_combobox, text="Filter by Warehouse Location")
        ...
        status_data = self.shared_functions.get_status_api()
        status_names = ["All"] + [item["name"] for item in status_data]
        self.status_combobox = Combobox(filter_button_frame, values=status_names, state="readonly", width=19,
                                        font=self.shared_functions.custom_font_size)
        self.status_combobox.set("All")
        self.status_combobox.grid(row=1, column=4, padx=(10, 0), pady=0, sticky=W)
        ToolTip(self.status_combobox, text="Filter by Raw Material Status")
        ...
        try:
            ...
            for item in data:
                record = (
                    ...
                    "{:,.2f}".format(float(item.get("qty", 0.0))),
                    item.get("status_name", ""),
                    datetime.fromisoformat(item.get("date_computed", "")).strftime("%m/%d/%Y") if item.get(
                        "date_computed") else ""
                )
                self.original_data.append(record)
                self.tree.insert("", END, values=record)
            ...
            messagebox.showerror("Connection Error",
                                 "Could not connect to the API server. Please check your network or server status.",
                                 parent=self.root)
        except requests.exceptions.Timeout:
            messagebox.showerror("Timeout Error", "The request to the API server timed out.", parent=self.root)
        except requests.exceptions.HTTPError as e:
            messagebox.showerror("API Error", f"HTTP Error: {e.response.status_code} - {e.response.text}",
                                 parent=self.root)
        except requests.exceptions.RequestException as e:
            messagebox.showerror("Request Error", f"An error occurred during the API request: {e}", parent=self.root)
        except Exception as e:
    ...
    def export_data(self):
        ...
        Fetch filtered data from API, generate Excel file locally using pandas,
        including detailed, overall daily summary, and per-warehouse daily summaries.
        Applies conditional formatting to highlight changes in quantity.
        """
        params = self._get_filter_params()
        if params is None:
            return
        ...
        try:
            ...
            if not raw_data:
                messagebox.showinfo("No Data", "No records found matching the filter criteria to export.",
                                    parent=self.root)
                return
            ...
            if df_detailed.empty:
                messagebox.showinfo("No Valid Dates", "No valid date entries found for aggregation in the export data.",
                                    parent=self.root)
                return

            # Create the overall daily RM summary (Sheet 2)
            df_long_summary_overall = df_detailed.groupby(['rm_code', df_detailed['date_computed'].dt.date])[
                'qty'].sum().reset_index()
            df_long_summary_overall = df_long_summary_overall.rename(columns={'date_computed': 'Date'})
            df_long_summary_overall['Date'] = df_long_summary_overall['Date'].apply(lambda x: x.strftime('%m/%d/%Y'))
            ...
            df_pivot_summary_overall = df_pivot_summary_overall.sort_index(ascending=True)
            sorted_date_cols_overall = pd.to_datetime(df_pivot_summary_overall.columns, format='%m/%d/%Y',
                                                      errors='coerce').sort_values().dropna()
            sorted_date_col_names_overall = sorted_date_cols_overall.strftime('%m/%d/%Y').tolist()
            df_pivot_summary_overall = df_pivot_summary_overall[sorted_date_col_names_overall]
            df_pivot_summary_overall = df_pivot_summary_overall.reset_index()
            ...
            with pd.ExcelWriter(excel_buffer, engine="xlsxwriter") as writer:
                ...
                # Adjust column widths and apply conditional formatting for overall summary sheet
                workbook = writer.book
                if 'Daily RM Summary' in writer.sheets:
                    summary_sheet_overall = writer.sheets['Daily RM Summary']
                    summary_sheet_overall.set_column(0, 0, 20)
                    ...
                    # Apply conditional formatting to overall summary sheet
                    self._apply_conditional_formatting(
                        workbook,
                        summary_sheet_overall,
                        df_pivot_summary_overall
                    )

                # --- Generate sheets per warehouse ---
                unique_warehouses = df_detailed[['wh_name', 'wh_number']].drop_duplicates().sort_values(by='wh_name')

                for index, wh_row in unique_warehouses.iterrows():
                    wh_name = wh_row['wh_name']
                    wh_number = wh_row['wh_number']
                    ...
                    if not df_wh_detailed.empty:
                        df_long_summary_wh = \
                        df_wh_detailed.groupby(['rm_code', df_wh_detailed['date_computed'].dt.date])[
                            'qty'].sum().reset_index()
                        df_long_summary_wh = df_long_summary_wh.rename(columns={'date_computed': 'Date'})
                        df_long_summary_wh['Date'] = df_long_summary_wh['Date'].apply(lambda x: x.strftime('%m/%d/%Y'))
                        ...
                        df_pivot_summary_wh = df_pivot_summary_wh.sort_index(ascending=True)
                        sorted_date_cols_wh = pd.to_datetime(df_pivot_summary_wh.columns, format='%m/%d/%Y',
                                                             errors='coerce').sort_values().dropna()
                        sorted_date_col_names_wh = sorted_date_cols_wh.strftime('%m/%d/%Y').tolist()
                        df_pivot_summary_wh = df_pivot_summary_wh[sorted_date_col_names_wh]
                        df_pivot_summary_wh = df_pivot_summary_wh.reset_index()
                        ...
                        if sheet_name in writer.sheets:
                            ...
                            # Apply conditional formatting to this warehouse sheet
                            self._apply_conditional_formatting(
                                workbook,
                                wh_sheet,
                                df_pivot_summary_wh
    ...
    def _apply_conditional_formatting(self, workbook, worksheet, df_pivot):
        """
        Applies conditional formatting to a worksheet based on value changes from the previous column.
        """
        # Define formats for highlighting
        green_format = workbook.add_format({'bg_color': '#C6EFCE', 'font_color': '#006100'})  # Light Green
        orange_format = workbook.add_format({'bg_color': '#FFC7CE', 'font_color': '#9C0006'})  # Light Orange

        # Get the number of rows and columns in the data part of the pivot table
        # (excluding header row and index column 'rm_code')
        num_rows = len(df_pivot)
        num_cols = len(df_pivot.columns)

        # Iterate through columns, starting from the second data column (index 2 in Excel, 1 in df_pivot after 'rm_code')
        # The first data column (index 1 in Excel, 0 in df_pivot after 'rm_code') is the first date, which should not be highlighted.
        # So we start comparison from the third column (Excel column C, index 2 in df_pivot).

        # Determine the range for conditional formatting.
        # Data starts from row 1 (Python index 0 for data, Excel row 2)
        # Data columns start from column 1 (Python index 1 for data, Excel column B)

        # The first data column (Excel column B, index 1 in df_pivot) is the "previous" for the second data column.
        # The actual data values start from row 1 (Excel row 2) and column 1 (Excel column B).

        # Loop through the date columns, starting from the second date column
        # In df_pivot, columns[0] is 'rm_code'.
        # columns[1] is the first date.
        # columns[2] is the second date, which is the first one we want to compare and highlight.

        # Loop through the columns that represent dates, starting from the second date column
        # df_pivot.columns[0] is 'rm_code'
        # df_pivot.columns[1] is the first date column (e.g., '07/01/2025')
        # df_pivot.columns[2] is the second date column (e.g., '07/02/2025')

        # We need to compare column 'j' with column 'j-1'
        # The loop for columns should start from the index of the second date column.
        # The column indices in xlsxwriter are 0-based.
        # df_pivot.columns[0] is 'rm_code' (Excel A)
        # df_pivot.columns[1] is the first date (Excel B)
        # df_pivot.columns[2] is the second date (Excel C)

        # Start from the second date column (index 2 in df_pivot.columns, which corresponds to Excel column C)
        for col_idx in range(2, num_cols):  # Loop from the 3rd column (index 2) to the last
            current_col_excel_idx = col_idx  # Excel column index (0-based)
            previous_col_excel_idx = col_idx - 1  # Excel column index of the previous date

            # Define the range for conditional formatting (e.g., $C$2:$C$100 for column C)
            # Data starts from row 1 (Python index 0), so Excel row 2.
            # Range for current column: from (row 1, current_col_excel_idx) to (last_row, current_col_excel_idx)
            # xlsxwriter uses 0-based row/col for add_conditional_format
            # (first_row, first_col, last_row, last_col)

            # The data in the DataFrame starts from row 0, which maps to Excel row 1 (after headers).
            # So, the data range for conditional formatting starts from row 1 (Excel row 2).
            # The range is (start_row, start_col, end_row, end_col)
            # start_row = 1 (Excel row 2, skipping header)
            # start_col = current_col_excel_idx
            # end_row = num_rows (Excel row num_rows + 1)
            # end_col = current_col_excel_idx

            # Example: for Excel column C (index 2), range would be (1, 2, num_rows, 2)

            # Formula for increasing value: current_cell > previous_cell
            # Example: =C2>B2
            # Use A1-style notation for formulas
            current_cell_ref = xlsxwriter.utility.xl_col_to_name(current_col_excel_idx) + '2'
            previous_cell_ref = xlsxwriter.utility.xl_col_to_name(previous_col_excel_idx) + '2'

            # Define the range for the conditional format, starting from the first data row (Excel row 2)
            # and going down to the last row of data.
            # The range should cover the entire column that we are applying the format to.
            cf_range = xlsxwriter.utility.xl_range(1, current_col_excel_idx, num_rows, current_col_excel_idx)

            # Rule for increasing value (Light Green)
            worksheet.conditional_format(
                cf_range,
                {'type': 'formula',
                 'criteria': f'={current_cell_ref}>{previous_cell_ref}',
                 'format': green_format}
            )

            # Rule for decreasing value (Light Orange)
            worksheet.conditional_format(
                cf_range,
                {'type': 'formula',
                 'criteria': f'={current_cell_ref}<{previous_cell_ref}',
