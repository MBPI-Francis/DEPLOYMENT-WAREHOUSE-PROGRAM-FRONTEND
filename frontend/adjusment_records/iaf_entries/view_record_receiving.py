...
class ReceivingRecord:
    ...
    def get_receiving_record(self,  record_id):
        ...
        try:
            ...
            response = requests.get(f"{server_ip}/api/receiving_reports/v1/list/historical/", params=params)

            if response.status_code == 200:

                receiving_record = response.json()
                if receiving_record:
                    filtered_record =  receiving_record[0]
...
# --------------------------------------------------[CORRECT DETAILS]---------------------------------------------------
    ...
    def view_record(self, item):
        ...
        message_label = ttk.Label(
            self.add_record_window,
            text="Receiving Inventory Adjustment Form - Type 1",
            justify="center",
            font=("Arial", 14, "bold"),
            bootstyle=SECONDARY
        )
